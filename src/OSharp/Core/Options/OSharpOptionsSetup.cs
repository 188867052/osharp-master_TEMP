...
namespace OSharp.Core.Options
{
    ...
    public class OsharpOptionsSetup : IConfigureOptions<OsharpOptions>
    {
        ...
        {
            this.SetDbContextOptions(options);

            IConfigurationSection section;

            // OAuth2
            section = this._configuration.GetSection("OSharp:OAuth2");
            ...
            // MailSender
            section = this._configuration.GetSection("OSharp:MailSender");
            MailSenderOptions sender = section.Get<MailSenderOptions>();
            if (sender != null)
            {
                if (sender.Password == null)
                {
                    sender.Password = this._configuration["OSharp:MailSender:Password"];
                }
            ...
            // JwtOptions
            section = this._configuration.GetSection("OSharp:Jwt");
            JwtOptions jwt = section.Get<JwtOptions>();
            if (jwt != null)
            {
                if (jwt.Secret == null)
                {
                    jwt.Secret = this._configuration["OSharp:Jwt:Secret"];
                }
            ...
            // RedisOptions
            section = this._configuration.GetSection("OSharp:Redis");
            ...
            // SwaggerOptions
            section = this._configuration.GetSection("OSharp:Swagger");
        ...
        {
            IConfigurationSection section = this._configuration.GetSection("OSharp:DbContexts");
            IDictionary<string, OsharpDbContextOptions> dict = section.Get<Dictionary<string, OsharpDbContextOptions>>();
            if (dict == null || dict.Count == 0)
            {
                string connectionString = this._configuration["ConnectionStrings:DefaultDbContext"];
