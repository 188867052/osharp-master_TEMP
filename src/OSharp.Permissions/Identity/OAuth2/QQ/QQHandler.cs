...
namespace OSharp.Identity.OAuth2.QQ
{
    internal class QQHandler : OAuthHandler<QQOptions>
    {
        ...
        {
            // 获取OpenId
            string openIdEndpoint = QueryHelpers.AddQueryString(this.Options.OpenIdEndpoint, "access_token", tokens.AccessToken);
            HttpResponseMessage openIdResponse = await this.Backchannel.GetAsync(openIdEndpoint, this.Context.RequestAborted);
            ...
            JObject json = JObject.Parse(this.企鹅的返回不拘一格传入这里统一转换为JSON(await openIdResponse.Content.ReadAsStringAsync()));
            ...
            string userInformationEndpoint = QueryHelpers.AddQueryString(this.Options.UserInformationEndpoint, parameters);
            HttpResponseMessage userInformationResponse = await this.Backchannel.GetAsync(userInformationEndpoint, this.Context.RequestAborted);
            ...
            OAuthCreatingTicketContext context = new OAuthCreatingTicketContext(new ClaimsPrincipal(identity), properties, this.Context, this.Scheme, this.Options, this.Backchannel, tokens, payload);
            ...
            // identity.AddClaim(new Claim(ClaimTypes.NameIdentifier, openId, ClaimValueTypes.String, Options.ClaimsIssuer));
            payload.Add("openid", openId);
            OAuthCreatingTicketContext context = new OAuthCreatingTicketContext(new ClaimsPrincipal(identity), properties, this.Context, this.Scheme, this.Options, this.Backchannel, tokens, document.RootElement);
            context.RunClaimActions();
            await this.Events.CreatingTicket(context);
            return new AuthenticationTicket(context.Principal, context.Properties, this.Scheme.Name);
#endif
        ...
        protected override async Task<OAuthTokenResponse> ExchangeCodeAsync(OAuthCodeExchangeContext context)
        {
            ...
            string endpoint = QueryHelpers.AddQueryString(this.Options.TokenEndpoint, parameters);
...
#if NETSTANDARD
            JObject payload = JObject.Parse(this.企鹅的返回不拘一格传入这里统一转换为JSON(await response.Content.ReadAsStringAsync()));
            return OAuthTokenResponse.Success(payload);
#else
            string json = this.企鹅的返回不拘一格传入这里统一转换为JSON(await response.Content.ReadAsStringAsync());
        ...
        private string 企鹅的返回不拘一格传入这里统一转换为JSON(string text)
        {
            ...
            */
            ...
            this.Logger.LogError($"未从JSON“{text}”中获取到有效的信息");
            return "{}";
        }
