...
namespace OSharp.Extensions.Tests
{
    public class ExpressionExtensionsTests : EntityTestBase
    {
        ...
        public void ComposeTest()
        {
            ...
            List<TestEntity> list1 = this.Entities.Where(m => m.IsDeleted && m.Id > 500).ToList();
            List<TestEntity> list2 = this.Entities.Where(p1.Compile()).ToList();
            Assert.True(list1.SequenceEqual(list2));
            list1 = this.Entities.Where(m => m.IsDeleted || m.Id > 500).ToList();
            list2 = this.Entities.Where(p2.Compile()).ToList();
            Assert.True(list1.SequenceEqual(list2));
        }
        ...
        public void AndTest()
        {
            ...
            List<TestEntity> list1 = this.Entities.Where(predicate.Compile()).ToList();
            List<TestEntity> list2 = this.Entities.Where(actual.Compile()).ToList();
            Assert.True(list1.SequenceEqual(list2));
        }
